 ...
 * - whisper-caption：cmake 构建转写 helper，并下载内置 Whisper Small + Silero VAD 模型
 *   （清单见 shared/captionModels.json）
 * - tts-helper：sherpa-onnx 预编译包 + clang++(darwin)/cmake(win32) 构建 TTS helper，
 *   并下载内置 TTS 模型（清单见 shared/ttsModels.json）
 * - 其他平台：无需构建，直接跳过
 */
...
if (process.platform === 'darwin') {
  if (!geometryOnly) {
    ...
    execFileSync(process.execPath, [join(root, 'native/whisper-caption/fetch-models.mjs')], { stdio: 'inherit' })
    execFileSync(process.execPath, [join(root, 'native/tts-helper/build.mjs')], { stdio: 'inherit' })
    execFileSync(process.execPath, [join(root, 'native/tts-helper/fetch-models.mjs')], { stdio: 'inherit' })
  }
