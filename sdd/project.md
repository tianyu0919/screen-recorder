...
| kr-03-background-export-queue | 后台导出队列与默认导出目录 | [background-export-queue/](./specs/screen-recorder/krs/kr-03-mp4-export/changes/background-export-queue/spec.md) | in_progress | kr-03-mp4-export | 实现已完成；macOS 已确认跨页面后台导出、首次展开/后续保持胶囊、进度与任务状态 UX。待文件命名/队列/export smoke、退出保护和 Windows 人工验收后转 completed。 |
...
| session-library-lazy-thumbnails | 会话库可视区懒加载与缩略图缓存 | [session-library-settings/changes/lazy-session-thumbnails/](./specs/screen-recorder/changes/session-library-settings/changes/lazy-session-thumbnails/spec.md) | in_progress | session-library-settings, ui-modern-light-motion-refresh | 首屏增量挂载会话卡片；可视区按需生成并持久化 WebP/时长缓存；静态卡片只读图片，悬停临时加载视频，永久删除时清理缓存。 |
| session-display-name | 录像显示名称与内联重命名 | [session-library-settings/changes/session-display-name/](./specs/screen-recorder/changes/session-library-settings/changes/session-display-name/spec.md) | in_progress | session-library-settings, kr-03-background-export-queue | 稳定 sessionId；详情页双击重命名；列表、MP4/WebM/SRT 命名同步，磁盘会话目录不改名。macOS 已通过人工冒烟，待 Windows 与重启恢复验证。 |
...
| kr-05-vfr-playback-smoothing | VFR 录屏流畅预览 | [preview-quality-control/changes/vfr-playback-smoothing/](./specs/screen-recorder/krs/kr-05-editor/changes/preview-stage-fit/changes/preview-quality-control/changes/vfr-playback-smoothing/spec.md) | in_progress | kr-02-motion-playback, kr-05-preview-quality-control | rVFC 仅上传新视频纹理，rAF 按媒体时钟连续合成运镜、字幕、波纹和播放头，兼容静态画面省帧。 |
