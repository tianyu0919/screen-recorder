...
| kr-03-background-export-queue | 后台导出队列与默认导出目录 | [background-export-queue/](./specs/screen-recorder/krs/kr-03-mp4-export/changes/background-export-queue/spec.md) | in_progress | kr-03-mp4-export | 实现已完成；文件命名、队列调度、typecheck/lint/build/export smoke 已通过（2026-08-26）。待退出保护和 macOS/Windows 人工导出验收后转 completed。 |
...
| kr-06-captions | M6 录制后离线字幕与编辑 | [krs/kr-06-captions/](./specs/screen-recorder/krs/kr-06-captions/spec.md) | in_progress | kr-01、kr-02、kr-03、kr-05 | 第一阶段按 change 收敛为编辑页从 `mic.wav` 本地离线生成字幕；后台任务、字幕轨、样式/位置、MP4 烧录与裁剪后 SRT；实时字幕留待后续。 |
| kr-07-voice-packs | M7 本地语音包与非破坏式变声 | [krs/kr-07-voice-packs/](./specs/screen-recorder/krs/kr-07-voice-packs/spec.md) | draft | kr-01、kr-03、kr-05 | 原声、低沉、清亮、广播、机器人本地 DSP 预设；Worker 等长派生 WAV、缓存、A/B 切换及预览/裁剪/导出一致；AI 音色与 TTS 不在一期（TTS 已由 kr-08 承接）。 |
| kr-08-tts-dubbing | 本地 TTS 配音（按字幕重读与无录音配音） | [krs/kr-08-tts-dubbing/](./specs/screen-recorder/krs/kr-08-tts-dubbing/spec.md) | in_progress | kr-01、kr-03、kr-05、kr-06 | sherpa-onnx 双平台统一本地默认引擎；字幕沿时间轴合成为 mic 位非破坏式派生轨；保调变速贴合、分段缓存及内置中英文音色；火山引擎云端高清接入作为独立 draft change 登记。 |
| kr-09-voice-conversion | 本地音色转换（保留韵律，二期占位） | [krs/kr-09-voice-conversion/](./specs/screen-recorder/krs/kr-09-voice-conversion/spec.md) | draft | kr-08 | RVC/seed-vc 类保留韵律换音色；复用 kr-08 mic 位派生轨基建；授权合规与 GPU/CPU 降级为二期前置澄清项。 |
...
| session-library-lazy-thumbnails | 会话库可视区懒加载与缩略图缓存 | [session-library-settings/changes/lazy-session-thumbnails/](./specs/screen-recorder/changes/session-library-settings/changes/lazy-session-thumbnails/spec.md) | completed | session-library-settings, ui-modern-light-motion-refresh | 首屏增量挂载会话卡片；可视区按需生成并持久化 WebP/时长缓存；静态卡片只读图片，悬停临时加载视频，永久删除时清理缓存。批次、缓存和生命周期 smoke 已通过（2026-08-26）。 |
| session-display-name | 录像显示名称与内联重命名 | [session-library-settings/changes/session-display-name/](./specs/screen-recorder/changes/session-library-settings/changes/session-display-name/spec.md) | completed | session-library-settings, kr-03-background-export-queue | 稳定 sessionId；详情页双击重命名；列表、MP4/WebM/SRT 命名同步，磁盘会话目录不改名。名称校验、Windows 保留名和导出命名 smoke 已通过（2026-08-26）。 |
...
| kr-05-preview-quality-control | 编辑预览清晰度与性能提醒 | [preview-stage-fit/changes/preview-quality-control/](./specs/screen-recorder/krs/kr-05-editor/changes/preview-stage-fit/changes/preview-quality-control/spec.md) | completed | kr-05-preview-stage-fit, kr-05-focus-preview, session-library-settings | 自动/流畅/高清/超清四档本机偏好；持续卡顿检测与顶部居中 Sonner 降档提示；会话卡片时长移入正文元数据行。preview-quality/render smoke 与 typecheck/lint/build 已通过（2026-08-26）。 |
| kr-05-vfr-playback-smoothing | VFR 录屏流畅预览 | [preview-quality-control/changes/vfr-playback-smoothing/](./specs/screen-recorder/krs/kr-05-editor/changes/preview-stage-fit/changes/preview-quality-control/changes/vfr-playback-smoothing/spec.md) | completed | kr-02-motion-playback, kr-05-preview-quality-control | rVFC 仅上传新视频纹理，rAF 按媒体时钟连续合成运镜、字幕、波纹和播放头，兼容静态画面省帧；自动验证已通过，双平台实机回归移交父级。 |
...
| kr-06-post-recording-captions | 录制后离线字幕生成与编辑 | [kr-06-captions/changes/post-recording-captions/](./specs/screen-recorder/krs/kr-06-captions/changes/post-recording-captions/spec.md) | completed | kr-06-captions, kr-01, kr-02, kr-03, kr-05 | 会话字幕默认关闭并持久化；Windows/macOS 内置 Small+VAD，支持自定义模型导入、长期选择、删除与缺失降级；以词级时间戳重组字幕，并统一预览、编辑、SRT 与烧录。caption/transcription smoke 与 typecheck/lint/build 已通过（2026-08-26）。 |
| kr-08-high-quality-bundled-voices | 高质量内置中英文 TTS 音色 | [kr-08-tts-dubbing/changes/high-quality-bundled-voices/](./specs/screen-recorder/krs/kr-08-tts-dubbing/changes/high-quality-bundled-voices/spec.md) | in_progress | kr-08-tts-dubbing | 内置 Kokoro 中英、Matcha 中文、Kokoro 英文三套模型；移除 Melo/Theresa/Fanchen 与官方下载路径；以带限重采样、WSOLA 修正和边界淡化消除电音、爆音与卡顿，不兼容本地测试期旧 TTS 会话。 |
| kr-08-volcengine-cloud-tts | 火山引擎云端高清 TTS | [kr-08-tts-dubbing/changes/volcengine-cloud-tts/](./specs/screen-recorder/krs/kr-08-tts-dubbing/changes/volcengine-cloud-tts/spec.md) | draft | kr-08-tts-dubbing, kr-08-high-quality-bundled-voices | 保持内置本地模型默认；用户自有新版 API Key 经 safeStorage 本机加密保存；可选精选中英文/自定义音色与资源 ID；首次文本外发及费用披露；分段缓存、原子结果与失败不混用本地音色。 |
...
kr-01/03/05/06 ────────────────────────────────────────────────────────> kr-08-tts-dubbing ──> kr-09-voice-conversion
```

执行顺序建议（见 [Epic tasks.md](./specs/screen-recorder/tasks.md)）：kr-01 → kr-02 → kr-03 为主线；kr-04 在 kr-01 完成后可与主线并行；kr-05 编辑闭环后可并行启动 kr-06 录制后字幕与 kr-07 本地语音包；kr-08 TTS 配音依赖 kr-06 字幕闭环，可随时启动；kr-09 音色转换为二期，依赖 kr-08 的派生轨基建。
