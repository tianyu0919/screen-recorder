...
- [ ] 已安装旧版可发现更新后的正式 GitHub Release，忽略 Draft 与 Pre-release。
- [ ] Windows 可由用户确认后完成下载，并在非录制状态下重启安装。
- [ ] macOS 未签名阶段只跳转对应 Release，不尝试应用内安装。
- [ ] 断网、元数据缺失或下载失败不影响录制、编辑、历史与导出。
- [ ] 自动检查设置在升级后保留，顶部入口和设置页状态一致。
