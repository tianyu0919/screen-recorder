...
- [ ] 1 分钟会话导出产出 1080p60 H.264 mp4，可被系统播放器正常打开
- [ ] 导出帧数 = 时长 × 60，帧率恒定（用 ffprobe 或播放器属性验证），无丢帧
- [ ] 导出视频与预览同时间点画面一致（抽查 ≥ 5 个时间点，含运镜进行中的帧）
- [ ] 含 mic.wav 的会话导出后声画同步（口型/敲击声与画面对齐）
- [ ] 高/低性能机器导出同一会话，输出帧内容与帧率一致（确定性）
- [ ] 导出进度条与已渲染帧数成比例，完成后显示保存路径
...
- [ ] 导出管线与预览复用同一合成器/相机求值代码路径（无复制粘贴的第二份实现）
- [ ] 导出期间 Renderer UI 不卡死（重活在 Worker 线程）
