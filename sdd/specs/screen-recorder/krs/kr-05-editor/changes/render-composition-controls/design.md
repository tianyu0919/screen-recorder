...
6. `backgroundColor` 保留最后合法选色；`resolveOutputPlan` 只在背景开启时把它作为有效合成色，关闭时统一返回默认非背景底色 `#16181D`，预览与导出消费同一有效值。
7. 导出前以目标尺寸探测编码器。若不支持，按设备可用上限逐级等比缩小到偶数宽高，再用最终尺寸创建画布、VideoEncoder 和 muxer；界面显示“实际输出 W×H”。
