...
- [x] 对话框取消无副作用；损坏文件给出友好提示（人工确认）
- [x] 波形块可拖动（不触发 seek），松手后位置固定（人工确认）
- [x] 预览：区间内发声、区间外静音、seek 后对齐（人工确认）
- [x] 导出：clip 位置/电平与预览一致，裁剪后仍对齐（人工确认）
- [x] 长音频导入后止于真实视频片尾，波形和导出均不越界（纯逻辑 smoke）
- [x] 波形左右边缘可裁掉音频头尾，主体仍可拖动定位，最短保留 100ms
...
- [x] 基础版性能验收结束；残余卡顿与多轨编辑体验转入 `changes/multi-track-audio-editing/`
- [x] 手动冒烟：Windows 平台
